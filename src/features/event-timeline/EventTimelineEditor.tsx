...
export function EventTimelineEditor({ durationMinutes, startTime, tracks, events, onChange }: {
  ...
  const saveEvent = (event: TimelineEvent) => { const normalized = { ...event, title: event.title.trim() || 'Untitled event', offsetMinutes: Math.min(durationMinutes, Math.max(0, event.offsetMinutes)), triggerBehavior: { ...event.triggerBehavior, launchActionsOnStart: event.type === 'action' }, updatedAt: nowIso() }; updateEvents(events.some((item) => item.id === event.id) ? events.map((item) => item.id === event.id ? normalized : item) : [...events, normalized]); setSelectedId(normalized.id); setEditing(null); };
...
function EventEditor({ event: initial, tracks, actions, onClose, onSave }: { event: TimelineEvent; tracks: TimelineTrack[]; actions: ReturnType<typeof useDeveloperActionStore.getState>['actions']; onClose: () => void; onSave: (event: TimelineEvent) => void }) {
  ...
  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onMouseDown={onClose}><div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
    ...
    <div className="grid min-h-0 flex-1 gap-5 overflow-y-auto p-6 lg:grid-cols-2">
      ...
      <label><span className="text-xs font-bold">Type</span><select value={event.type} onChange={(e) => { const type = e.target.value as TimelineEventType; patch({ type, triggerBehavior: { ...event.triggerBehavior, launchActionsOnStart: type === 'action' } }); }} className="mt-1 w-full rounded-lg border border-border p-2.5">{eventTypes.map((type) => <option key={type} value={type}>{type.replace('_', ' ')}</option>)}</select></label>
      ...
      <label className="sm:col-span-2"><span className="text-xs font-bold">{event.type === 'agent' ? 'Description / prompt' : 'Description'}</span><textarea value={event.description || ''} onChange={(e) => patch({ description: e.target.value })} className="mt-1 min-h-28 w-full resize-y rounded-lg border border-border p-3" placeholder={event.type === 'agent' ? 'Tell the AI agent what to do when this event starts.' : 'Optional context shown when this event starts'} /></label>
      ...
      {event.type === 'agent' && <div className="sm:col-span-2 rounded-xl border border-fuchsia-500/30 bg-fuchsia-500/10 p-4">
        <label><span className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-fuchsia-200"><Bot className="h-4 w-4" /> AI agent</span><select value={event.agentProfileId || ''} onChange={(e) => patch({ agentProfileId: e.target.value || undefined })} className="mt-2 w-full rounded-lg border border-border p-2.5"><option value="">Select an enabled agent profile</option>{agentProfiles.filter((profile) => profile.enabled).map((profile) => <option key={profile.id} value={profile.id}>{profile.name} · {profile.command}</option>)}</select></label>
        <p className="mt-2 text-xs text-text-secondary">When this event starts, ChronoFlow sends the event description plus schedule/session context to the selected CLI agent and saves stdout/stderr in Agent runs.</p>
      </div>}
