...
export function PlannerWidget({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  return <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-surface p-6 shadow-sm md:p-8">
    ...
    <div className="grid min-h-0 flex-1 gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(300px,.85fr)]">
      <div className="flex min-w-0 min-h-0 flex-col rounded-xl border border-border bg-surface-muted p-3">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-7 gap-1" style={{ gridTemplateRows: `repeat(${Math.ceil(calendarDays.length / 7)}, minmax(0, 1fr))` }}>{calendarDays.map((day) => {
          ...
          return <button key={key} onClick={() => selectDay(day)} className={clsx('relative flex min-h-11 h-full flex-col items-center justify-center rounded-lg text-sm font-bold transition', !isSameMonth(day, currentMonth) && 'opacity-30', selected ? 'bg-primary text-primary-fg' : 'hover:bg-surface-hover', isToday(day) && !selected && 'ring-1 ring-primary/60')}>
            {format(day, 'd')}
      ...
      <div className="flex min-h-0 flex-col rounded-xl border border-border bg-surface-muted p-4">
        ...
        <div className="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">{dayTasks.map((task) => <div key={task.id} className="group grid grid-cols-[auto_72px_1fr_auto] items-center gap-3 rounded-lg border border-border bg-surface p-3">
